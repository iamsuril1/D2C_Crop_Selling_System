...
const Navbar = () => {
  ...
  // Pending return count for farmer badge
  ...
  // Poll pending returns count for farmers
  useEffect(() => {
    ...
    if (user.role === "farmer") {
      return (
        <>
          ...
          {/* NEW: Returns link with pending badge */}
      ...
      <div className="flex items-center space-x-4">
                    ...
                    {notifications.slice(0, 8).map((n) => (
                      <button
                        ...
                        onClick={() => {
                          ...
                          closeAll();
                          if (n?.data?.orderId) {
                            navigate(user?.role === "consumer" ? "/my-orders" : "/farmer/orders");
                          } else if (n?.data?.returnId) {
                            navigate(user?.role === "consumer" ? "/my-orders" : "/farmer/returns");
                          }
                        }}
        ...
        {user ? (
          <div className="relative" ref={profileRef}>
            ...
            />
            {profileOpen && (
              <div className="absolute right-0 mt-2 bg-white shadow-lg rounded-md w-48 z-50 border">
                <Link to="/profile"       className="block px-4 py-2 hover:bg-gray-100" onClick={closeAll}>Profile</Link>
                <button type="button" onClick={goToDashboard} className="block w-full text-left px-4 py-2 hover:bg-gray-100">Dashboard</button>
                {user.role === "consumer" && (
                  <Link to="/my-orders" className="block px-4 py-2 hover:bg-gray-100" onClick={closeAll}>My orders</Link>
                )}
                {user.role === "farmer" && (
                  <>
                    <Link to="/farmer/orders"  className="block px-4 py-2 hover:bg-gray-100" onClick={closeAll}>Manage orders</Link>
                    <Link to="/farmer/returns" className="block px-4 py-2 hover:bg-gray-100" onClick={closeAll}>
                      Returns
                      {pendingReturns > 0 && (
                ...
                <Link to="/notifications" className="block px-4 py-2 hover:bg-gray-100" onClick={closeAll}>
                  Notifications
                  {hasUnread ? <span className="ml-2 text-xs text-red-600 font-semibold">({unreadCount})</span> : null}
                </Link>
                <button type="button" onClick={handleLogout} className="block w-full text-left px-4 py-2 hover:bg-red-50 text-red-600">Logout</button>
              </div>
            )}
          </div>
        ) : (
        ...
        <div className="fixed inset-0 bg-black bg-opacity-40 z-50 md:hidden">
          <div className="bg-white w-64 h-full p-6 overflow-y-auto">
            ...
            <div className="flex flex-col space-y-4">
              {user?.role === "consumer" && (
                <>
                  <Link to="/consumer"  onClick={closeAll}>View products</Link>
                  <Link to="/my-orders" onClick={closeAll} className="flex items-center gap-2 font-medium"><FaClipboardList className="text-lg" />My orders</Link>
                  <Link to="/cart"      onClick={closeAll} className="flex items-center gap-2"><FaShoppingCart className="text-lg" />Cart {cartCount > 0 && `(${cartCount})`}</Link>
                </>
              )}
              {user?.role === "farmer" && (
                <>
                  <Link to="/add-product"    onClick={closeAll}>Add product</Link>
                  <Link to="/farmer/orders"  onClick={closeAll} className="flex items-center gap-2 font-medium"><FaBox className="text-lg" />Manage orders</Link>
                  <Link to="/farmer/returns" onClick={closeAll} className="flex items-center justify-between font-medium">
                    <span className="flex items-center gap-2"><FaUndo className="text-lg" />Returns</span>
                    {pendingReturns > 0 && (
                      <span className="bg-orange-500 text-white text-xs px-2 py-0.5 rounded-full">{pendingReturns}</span>
                    )}
                  </Link>
                </>
              )}
              ...
              {!user ? (
                ...
                <>
                  ...
                  <Link to="/notifications" onClick={closeAll} className="flex items-center justify-between">
                    ...
                    {hasUnread && <span className="bg-red-500 text-white text-xs px-2 py-0.5 rounded-full">{unreadCount}</span>}
                  </Link>
                  <button type="button" onClick={handleLogout} className="text-red-600">Logout</button>
                </>
              )}
            </div>
          </div>
