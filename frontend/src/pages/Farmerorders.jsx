    ...
    <div className="min-h-screen bg-gray-50 px-3 sm:px-4 md:px-8 py-5 sm:py-8">
      ...
      <div className="max-w-6xl mx-auto">
        ...
        <div className="mb-5 sm:mb-6">
          <h1 className="text-2xl sm:text-3xl font-bold text-gray-900 mb-1">Order Management</h1>
          <p className="text-gray-500 text-sm">Tap any order card to manage it</p>
        </div>
        ...
        <div className="bg-white rounded-xl shadow-sm p-1.5 mb-5 sm:mb-6 flex gap-1 overflow-x-auto border border-gray-100 scrollbar-none">
          {["all", "pending", "confirmed", "shipped", "delivered", "cancelled"].map((status) => (
            <button
              key={status}
              onClick={() => setFilter(status)}
              className={`px-3 py-1.5 rounded-lg text-xs font-medium whitespace-nowrap flex-shrink-0 sm:flex-1 transition ${
        ...
        {filteredOrders.length === 0 ? (
          ...
          <>
            {/* Order cards — 2 cols on mobile, 4 on sm+ */}
                  ...
                  <div key={orderId} className="relative">
                    <button
                      ...
                      className={`w-full aspect-square flex flex-col justify-between rounded-2xl p-3 sm:p-4 text-left transition-all duration-200 border-2 ${
                        isSelected
                          ...
                          : "border-gray-100 bg-white hover:border-green-300 hover:shadow-md active:scale-95"
                      }`}
                    >
                      <div className="flex items-start justify-between gap-1">
                        <div className={`w-2 h-2 sm:w-2.5 sm:h-2.5 rounded-full mt-0.5 flex-shrink-0 ${getStatusDot(order.status)}`} />
                        <div className="flex flex-col items-end gap-1">
                          <span className={`text-[10px] sm:text-xs font-semibold px-1.5 sm:px-2 py-0.5 rounded-full ${getStatusBadge(order.status)}`}>
                            {order.status?.charAt(0).toUpperCase() + order.status?.slice(1)}
                          </span>
                          {paymentPending && order.status !== "cancelled" && (
                            <span className="text-[10px] sm:text-xs font-semibold px-1.5 py-0.5 rounded-full bg-yellow-100 text-yellow-700">
                              No Pay
                            </span>
                          )}
                        </div>
                      ...
                      <div>
                        <div className="text-lg sm:text-xl font-bold text-gray-900">#{orderDisplayId}</div>
                        <div className="text-[11px] sm:text-xs text-gray-500 truncate mt-0.5">{consumerName}</div>
                    ...
                    {cancellable && !paymentPending && (
                      <button
                        ...
                        className="absolute bottom-2 right-2 sm:bottom-3 sm:right-3 text-[10px] sm:text-xs bg-red-50 hover:bg-red-100 active:bg-red-200 text-red-600 font-semibold px-1.5 sm:px-2 py-1 rounded-lg transition disabled:opacity-50"
                      >
            ...
            {expandedOrder && (() => {
              ...
              return (
                <div className="bg-white rounded-2xl border-2 border-green-500 shadow-xl overflow-hidden mt-2">
                  {/* Header */}
                  <div className="bg-gray-50 px-4 sm:px-6 py-4 sm:py-5 border-b">
                    <div className="flex items-start justify-between gap-3">
                      <div className="min-w-0">
                        <div className="flex flex-wrap items-center gap-2 mb-1">
                          ...
                          <span className={`px-2 sm:px-3 py-0.5 sm:py-1 rounded-full text-xs font-bold ${getStatusBadge(order.status)}`}>
                        ...
                        <p className="text-sm text-gray-500 truncate">
                          ...
                          {consumer?.email && (
                            <span className="ml-1 text-gray-400 hidden sm:inline">({consumer.email})</span>
                          )}
                        </p>
                      ...
                      <div className="flex items-start gap-2 flex-shrink-0">
                        ...
                        <button
                          ...
                          className="w-8 h-8 rounded-full bg-gray-100 hover:bg-gray-200 active:bg-gray-300 flex items-center justify-center text-gray-500 text-sm font-bold transition flex-shrink-0"
                        >
                          ✕
                        </button>
                      </div>
                        ...
                        {statusSteps.map((s) => (
                          <span
                            ...
                            className={`capitalize font-medium ${
                              statusSteps.indexOf(order.status) >= statusSteps.indexOf(s) ? "text-gray-800" : ""
                            }`}
                          >
                            {s}
                          </span>
                  ...
                  <div className="p-4 sm:p-6 space-y-4">
                      ...
                      <div>
                        ...
                        <div className="space-y-1.5">
                          {myShipment.items?.map((item, idx) => (
                            <div
                              key={idx}
                              className="flex justify-between items-start sm:items-center bg-gray-50 rounded-lg px-3 py-2.5 text-sm gap-2"
                            >
                              <div className="min-w-0">
                                <span className="font-medium text-gray-900 block sm:inline">{item.name}</span>
                                <span className="text-gray-400 text-xs block sm:inline sm:ml-2">
                                  ×{item.quantity} · Rs.{item.price} each
                                </span>
                                {item.orderType === "bulk" && (
                                  <span className="ml-0 sm:ml-2 mt-1 sm:mt-0 inline-block text-xs bg-amber-100 text-amber-700 font-semibold px-1.5 py-0.5 rounded-full">
                                    Bulk
                                  </span>
                                )}
                              </div>
                    ...
                    {myShipment && (
                      <div className="bg-blue-50 border border-blue-200 rounded-xl p-3 sm:p-4">
                        ...
                        {paymentPending && (
                          <div className="mt-3 bg-yellow-50 border border-yellow-200 rounded-xl px-3 sm:px-4 py-2.5 sm:py-3 text-xs sm:text-sm text-yellow-800 text-center">
                            ⏳ Waiting for consumer to complete payment
                          </div>
                        )}
                        {myShipment.paymentStatus === "paid" && (
                          <div className="mt-3 bg-green-100 border border-green-300 rounded-xl px-3 sm:px-4 py-2.5 sm:py-3 text-xs sm:text-sm text-green-800 text-center">
                            ✓ Payment confirmed — Rs.{myShipment.subtotal} will be paid to you by admin
                          </div>
                        )}
                        {myShipment.paymentStatus === "failed" && (
                          <div className="mt-3 bg-red-50 border border-red-200 rounded-xl px-3 sm:px-4 py-2.5 sm:py-3 text-xs sm:text-sm text-red-800 text-center">
                    ...
                    <div className="bg-gray-50 rounded-xl p-3 sm:p-4">
                      ...
                      {order.status === "cancelled" ? (
                        ...
                        <div className="bg-yellow-50 border border-yellow-200 rounded-xl px-4 py-3 text-xs sm:text-sm text-yellow-800 text-center">
                        ...
                        <div className="flex flex-wrap gap-2">
                          {order.status === "pending" && (
                            <button
                              ...
                              disabled={updatingOrder === orderId}
                              className="flex-1 sm:flex-none bg-blue-600 hover:bg-blue-700 active:bg-blue-800 disabled:bg-blue-400 text-white font-semibold px-4 sm:px-5 py-2.5 sm:py-2 rounded-lg text-sm transition"
                            >
                              Confirm Order
                            </button>
                          )}
                          {order.status === "confirmed" && (
                            <button
                              ...
                              disabled={updatingOrder === orderId}
                              className="flex-1 sm:flex-none bg-purple-600 hover:bg-purple-700 active:bg-purple-800 disabled:bg-purple-400 text-white font-semibold px-4 sm:px-5 py-2.5 sm:py-2 rounded-lg text-sm transition"
                            >
                              Mark as Shipped
                            </button>
                          )}
                          {order.status === "shipped" && (
                            <button
                              ...
                              disabled={updatingOrder === orderId}
                              className="flex-1 sm:flex-none bg-green-600 hover:bg-green-700 active:bg-green-800 disabled:bg-green-400 text-white font-semibold px-4 sm:px-5 py-2.5 sm:py-2 rounded-lg text-sm transition"
                            >
                              Mark as Delivered
                            </button>
                          )}
                          {cancellable && (
                            <button
                              ...
                              disabled={updatingOrder === orderId}
                              className="flex-1 sm:flex-none bg-red-600 hover:bg-red-700 active:bg-red-800 disabled:bg-red-400 text-white font-semibold px-4 sm:px-5 py-2.5 sm:py-2 rounded-lg text-sm transition"
                            >
                              Cancel Order
                            </button>
                          )}
