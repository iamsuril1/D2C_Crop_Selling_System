...
export default function Orders() {
  ...
  const { cartItems, removeFromCart, updateQty, clearCart } =
    useContext(CartContext); // ADDED clearCart [file:47]
  ...
  // Unique farmers shipments count; delivery is 200 per shipment
  const shipmentCount = useMemo(() => {
    const ids = new Set();
    for (const it of cartItems || []) {
      const farmerId = it?.farmer?.id || it?.farmer?._id || it?.farmerId;
  ...
  const deliveryCharge = useMemo(
    () => shipmentCount * DELIVERY_PER_SHIPMENT,
    [shipmentCount]
  );
  ...
  const buildProductId = (item) => item?.productId || item?.id;
  ...
  const handleCheckout = async () => {
    ...
    try {
      ...
      const res = await api.post("/api/orders", { items }); // matches your backend shape [file:47]
                    ...
                    <div className="mt-3 flex items-center justify-between">
                      <div className="flex items-center border rounded-xl overflow-hidden">
                        <button
                          ...
                          onClick={() =>
                            updateQty(item?.id, Number(item?.quantity || 1) - 1)
                          }
                        >
                          -
                        </button>
                      ...
                      <div className="text-right">
                        ...
                        <div className="text-xs text-gray-500">
                          Rs. {Number(item?.price || 0)} per {item?.unit || "unit"}
                        </div>
                      </div>
                    </div>
                  </div>
