      ...
      {bankTransfer && (
        <div className={`rounded-xl p-3 border ${preferred === "bank_transfer" ? "border-blue-300 bg-blue-50" : "border-gray-100 bg-gray-50"}`}>
          ...
          <div className="space-y-1 text-sm">
            <div className="flex justify-between"><span className="text-gray-500">Bank</span><span className="font-semibold">{bankTransfer.bankName}</span></div>
            <div className="flex justify-between"><span className="text-gray-500">Account no.</span><span className="font-mono font-bold">{bankTransfer.accountNumber}</span></div>
            <div className="flex justify-between"><span className="text-gray-500">Name</span><span className="font-semibold">{bankTransfer.accountName}</span></div>
            {bankTransfer.bankBranch && <div className="flex justify-between"><span className="text-gray-500">Branch</span><span className="text-gray-700">{bankTransfer.bankBranch}</span></div>}
          </div>
        </div>
      )}
    </div>
  ...
  return (
    <div className="flex items-center gap-1.5 bg-orange-50 border border-orange-200 rounded-lg px-3 py-1.5">
      ...
      <span className="text-xs font-semibold text-orange-700">
        Cooldown: {cooldown.daysLeft} day{cooldown.daysLeft !== 1 ? "s" : ""} left
      </span>
    </div>
  );
};
          ...
          {blocked && (
            <div className="bg-orange-50 border-2 border-orange-300 rounded-xl p-4">
              <div className="flex items-center gap-2 mb-2">
                ...
                <p className="font-bold text-orange-800">15-Day Cooldown Active</p>
              </div>
              <p className="text-sm text-orange-700">
                Last paid on {new Date(cooldown.lastPaidAt).toLocaleDateString()}.
                Next payout allowed in <strong>{cooldown.daysLeft} day{cooldown.daysLeft !== 1 ? "s" : ""}</strong>
                {cooldown.nextPayoutAt && ` (${new Date(cooldown.nextPayoutAt).toLocaleDateString()})`}.
              </p>
              <p className="text-xs text-orange-600 mt-2">
                This prevents duplicate payments. The cooldown ensures farmers receive one consolidated payout per cycle.
              </p>
            </div>
          )}
      ...
      <div className="max-w-5xl mx-auto">
        ...
        <div className="mb-8 flex items-center justify-between flex-wrap gap-4">
          <div>
            ...
            <p className="text-gray-500 mt-1 text-sm">Pay each farmer their accumulated balance · 15-day cooldown enforced</p>
          </div>
          <button onClick={() => navigate("/admin")}
            className="text-sm border border-gray-200 px-4 py-2 rounded-xl hover:bg-gray-50 transition text-gray-600">
            ← Back to Dashboard
          </button>
        </div>
        ...
        <div className="bg-blue-50 border border-blue-200 rounded-xl p-4 mb-6 flex items-start gap-3">
          ...
          <div className="text-sm text-blue-800">
            ...
            <p className="text-blue-700 mt-0.5">Each farmer can only receive a payout once every 15 days. This prevents duplicate payments and ensures accumulated balances are settled in one transfer. Farmers on cooldown will show a countdown timer.</p>
          </div>
        </div>
                    ...
                    >
                      ...
                      <div className="px-6 py-5 flex items-center gap-4 flex-wrap">
                        ...
                        <div className="text-right">
                          <p className={`text-2xl font-black ${blocked ? "text-gray-400" : "text-green-700"}`}>
                        ...
                        <div className="flex items-center gap-2 flex-shrink-0">
                          ...
                          <button
                            ...
                            className={`font-bold px-4 py-1.5 rounded-lg text-sm transition shadow-sm ${
                              blocked
                                    ...
                                    <div className="text-right">
                                      {s.returnDeduction > 0 ? (
                                        <div>
                                          ...
                                          <p className="text-xs text-red-500">-Rs.{s.returnDeduction} refund deducted</p>
                                        </div>
                                      ) : (
                          ...
                          {farmer.payments.map((p, i) => (
                            <div key={i} className="flex items-center justify-between bg-gray-50 rounded-xl px-4 py-3 text-sm">
                              <div className="flex items-center gap-3">
                                ...
                                <div>
                                  {p.reference && <p className="text-xs font-mono text-gray-600">Ref: {p.reference}</p>}
