/* src/pages/FarmerPaymentSettings.jsx
   Payment methods available to farmers for RECEIVING money:
   - eSewa (digital wallet)
   - Bank QR (scan to pay)
   - Bank Transfer (direct)

   NOTE: Cash on Delivery is a consumer-facing payment option chosen
   by buyers at checkout. It is NOT configured here because the farmer
   has no account to set up for it — they simply accept cash on delivery
   automatically.
*/
...
const FarmerPaymentSettings = () => {
  ...
  /* Only the three methods a farmer needs an account/ID for */
    ...
    <div className="min-h-screen bg-gray-50 py-12 px-4">
      ...
      <div className="max-w-3xl mx-auto">
        <div className="bg-white rounded-2xl shadow-lg p-8">
          ...
          {/* Info banner */}
          <div className="mb-8 bg-blue-50 border border-blue-200 rounded-xl p-5 space-y-3">
            <p className="text-sm font-semibold text-blue-900">
              How admin payouts work
            </p>
          ...
          <div className="space-y-6">
            ...
            {/* ── eSewa ── */}
            <div className={`border rounded-xl p-6 transition ${esewaMethod?.enabled ? "bg-green-50 border-green-300" : "bg-white border-gray-200"}`}>
              <div className="flex items-center gap-3 mb-4">
                <input type="checkbox" checked={esewaMethod?.enabled || false}
              ...
              {esewaMethod?.enabled && (
                <div className="ml-8 space-y-3">
                  <div>
                    ...
                    />
                    {errors.esewa_esewaId && <p className="text-red-500 text-xs mt-1">{errors.esewa_esewaId}</p>}
            ...
            {/* ── Bank QR ── */}
            <div className={`border rounded-xl p-6 transition ${bankQRMethod?.enabled ? "bg-purple-50 border-purple-300" : "bg-white border-gray-200"}`}>
              <div className="flex items-center gap-3 mb-4">
                <input type="checkbox" checked={bankQRMethod?.enabled || false}
              ...
              {bankQRMethod?.enabled && (
                <div className="ml-8 space-y-4">
                  <div>
                    ...
                    <select value={bankQRMethod.bankName || ""}
                      onChange={(e) => updateMethodField("bank_qr", "bankName", e.target.value)}
                      className={`w-full border rounded-lg px-4 py-2.5 text-sm ${errors.bank_qr_bankName ? "border-red-500" : "border-gray-300"}`}>
                      <option value="">Select Bank</option>
                      {BANKS.map((b) => <option key={b} value={b}>{b}</option>)}
                    </select>
                    {errors.bank_qr_bankName && <p className="text-red-500 text-xs mt-1">{errors.bank_qr_bankName}</p>}
                  </div>
                  ...
                  <div>
                    ...
                    <input type="file" accept="image/*" onChange={handleQRUpload}
                      className="w-full border rounded-lg px-4 py-2 text-sm file:mr-4 file:py-2 file:px-4 file:rounded-lg file:border-0 file:bg-green-50 file:text-green-700 hover:file:bg-green-100" />
                    {errors.qr && <p className="text-red-500 text-xs mt-1">{errors.qr}</p>}
                    {errors.bank_qr_qrCodeImage && <p className="text-red-500 text-xs mt-1">{errors.bank_qr_qrCodeImage}</p>}
            ...
            {/* ── Bank Transfer ── */}
            <div className={`border rounded-xl p-6 transition ${bankTransferMethod?.enabled ? "bg-blue-50 border-blue-300" : "bg-white border-gray-200"}`}>
              <div className="flex items-center gap-3 mb-4">
                <input type="checkbox" checked={bankTransferMethod?.enabled || false}
              ...
              {bankTransferMethod?.enabled && (
                <div className="ml-8 grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div>
                    ...
                    <select value={bankTransferMethod.bankName || ""}
                      onChange={(e) => updateMethodField("bank_transfer", "bankName", e.target.value)}
                      className={`w-full border rounded-lg px-4 py-2.5 text-sm ${errors.bank_transfer_bankName ? "border-red-500" : "border-gray-300"}`}>
                      <option value="">Select Bank</option>
                      {BANKS.map((b) => <option key={b} value={b}>{b}</option>)}
                    </select>
                    {errors.bank_transfer_bankName && <p className="text-red-500 text-xs mt-1">{errors.bank_transfer_bankName}</p>}
                  </div>
                  <div>
                    <label className="block text-sm font-medium mb-2">Account Number <span className="text-red-500">*</span></label>
                    <input type="text"
                      value={bankTransferMethod.accountNumber || ""}
                      onChange={(e) => updateMethodField("bank_transfer", "accountNumber", e.target.value)}
                      placeholder="XXXXXXXXXXXX"
                      className={`w-full border rounded-lg px-4 py-2.5 text-sm ${errors.bank_transfer_accountNumber ? "border-red-500" : "border-gray-300"}`} />
                    {errors.bank_transfer_accountNumber && <p className="text-red-500 text-xs mt-1">{errors.bank_transfer_accountNumber}</p>}
                  </div>
                  <div>
                    ...
                    <input type="text"
                      ...
                      className={`w-full border rounded-lg px-4 py-2.5 text-sm ${errors.bank_transfer_accountName ? "border-red-500" : "border-gray-300"}`} />
                    {errors.bank_transfer_accountName && <p className="text-red-500 text-xs mt-1">{errors.bank_transfer_accountName}</p>}
                  </div>
                  <div>
            ...
            <div className="border-t pt-6">
              <div className="bg-amber-50 border border-amber-200 rounded-xl p-5">
                ...
                <p className="text-xs text-gray-500 mb-3">
                  Admin will use this method by default when paying out your earnings. They can still choose any other enabled method.
                </p>
                <select value={preferredMethod}
                  onChange={(e) => setPreferredMethod(e.target.value)}
                  className={`w-full border rounded-lg px-4 py-2.5 text-sm ${errors.preferred ? "border-red-500" : "border-gray-300"}`}>
                  <option value="esewa">eSewa</option>
                  <option value="bank_qr">Bank QR</option>
                  <option value="bank_transfer">Bank Transfer</option>
                </select>
                {errors.preferred && <p className="text-red-500 text-xs mt-1">{errors.preferred}</p>}
              </div>
            </div>
          ...
          <div className="flex gap-4 mt-8 pt-6 border-t">
            <button onClick={handleSave} disabled={saving}
              className="flex-1 bg-green-600 hover:bg-green-700 text-white font-semibold py-3 rounded-xl disabled:opacity-50 transition">
              {saving ? (
            ...
            <button onClick={() => navigate("/farmer")} disabled={saving}
