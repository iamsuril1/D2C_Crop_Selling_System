        ...
        <div className="p-6 space-y-5">
          ...
          <div className="bg-gray-50 rounded-xl p-4 space-y-2">
            ...
            {returnDoc.refundMethod === "esewa" && detail.esewaId && (
              <p className="text-sm font-mono font-bold text-gray-900">eSewa ID: {detail.esewaId}</p>
            )}
            {returnDoc.refundMethod === "bank_transfer" && (
              <div className="text-sm space-y-0.5">
                ...
                <p><span className="text-gray-500">Account: </span><span className="font-mono font-bold">{detail.accountNumber}</span></p>
...
const AdminReturns = () => {
  ...
  const [refundTarget, setRefundTarget] = useState(null); // return doc to refund
      ...
      <div className="max-w-6xl mx-auto">
        ...
        {stats && (
          <div className="grid grid-cols-2 lg:grid-cols-6 gap-3 mb-8">
            {[
              { label: "Total Returns",    value: stats.total,           color: "text-gray-900",   bg: "bg-white",       border: "border-gray-200"   },
              { label: "Pending Review",   value: stats.pendingApproval, color: "text-yellow-700", bg: "bg-yellow-50",   border: "border-yellow-200" },
              { label: "Approved",         value: stats.approved,        color: "text-green-700",  bg: "bg-green-50",    border: "border-green-200"  },
              { label: "Refund Pending",   value: stats.refundPending,   color: "text-orange-700", bg: "bg-orange-50",   border: "border-orange-200" },
              { label: "Refunds Done",     value: stats.refundProcessed, color: "text-blue-700",   bg: "bg-blue-50",     border: "border-blue-200"   },
        ...
        {/* Filter tabs */}
        <div className="bg-white rounded-xl border border-gray-100 shadow-sm p-1.5 mb-6 flex gap-1 overflow-x-auto">
          {[
            { value: "all",           label: "All Returns",       count: returns.length },
            { value: "pending",       label: "Pending Review",    count: returns.filter(r => r.status === "pending").length },
            { value: "approved",      label: "Approved",          count: returns.filter(r => r.status === "approved").length },
            { value: "refund_pending",label: "Refund Pending",    count: returns.filter(r => r.status === "approved" && r.refundStatus === "pending").length },
            { value: "refund_done",   label: "Refund Processed",  count: returns.filter(r => r.refundStatus === "processed").length },
                ...
                >
                  ...
                  >
                    <div className="flex items-center gap-3 min-w-0">
                      ...
                      <div className="min-w-0">
                        <p className="font-semibold text-gray-900 text-sm truncate">
                          {consumerName}
                          <span className="ml-2 text-gray-400 font-normal text-xs">→ {farmerName}</span>
                    ...
                    <div className="flex items-center gap-2 flex-shrink-0 flex-wrap justify-end">
                      ...
                      {ret.status === "approved" && (
                        <span className={`text-xs font-semibold px-2.5 py-1 rounded-full ${rs.badge}`}>
                          {rs.label}
                        </span>
                      )}
                      {needsRefund && (
                  ...
                  {/* Expanded */}
                  {isExpanded && (
                    <div className="border-t border-gray-100 px-5 py-5 grid md:grid-cols-2 gap-6">
                          ...
                          <div className="bg-green-50 border border-green-200 rounded-xl p-4">
                            ...
                            <div className="text-sm space-y-0.5 text-green-800">
                              ...
                              {ret.refundRecord.reference && (
                                <p><span className="text-gray-500">Ref: </span><span className="font-mono font-bold">{ret.refundRecord.reference}</span></p>
                              )}
